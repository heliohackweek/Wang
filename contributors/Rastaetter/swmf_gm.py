...
class SWMF_GM(Kamodo):
    def __init__(self,
        ...
        self.var = vars[0]
        for var in vars:
            self.variables[var] = dict(units = units[vars.index(var)], data = np.array(mhd[var], dtype='float32') )
        number_of_cells = (np.array(mhd['x']).shape)[0]
        cell_volume=np.zeros(number_of_cells,dtype='float32')
        block_nx=np.int32(mhd.meta['NX'])
        block_ny=np.int32(mhd.meta['NY'])
        block_nz=np.int32(mhd.meta['NZ'])
        block_nv=block_nx*block_ny
        block_size=block_nv*block_nz  # this should come from metadata as well...
        for i in np.arange(0,number_of_cells,block_size,dtype=np.int32):
            dx=mhd['x'][i+1]       -mhd['x'][i]
            dy=mhd['y'][i+block_nx]-mhd['y'][i]
            dz=mhd['z'][i+block_nv]-mhd['z'][i]
            cell_volume[i:i+block_size]=dx*dy*dz
        self.variables['volume'] =dict(units='R**3',data=cell_volume)
